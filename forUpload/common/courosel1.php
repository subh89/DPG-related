<?php
echo '<div id="myCarousel" class="carousel slide" data-ride="carousel">
      <!-- Indicators -->
      <ol class="carousel-indicators">
        <li data-target="#myCarousel" data-slide-to="0" class=""></li>
        <li data-target="#myCarousel" data-slide-to="1" class=""></li>
        <li data-target="#myCarousel" data-slide-to="2" class=""></li>
        <li data-target="#myCarousel" data-slide-to="3" class="active"></li>
        <li data-target="#myCarousel" data-slide-to="4" class=""></li>
      </ol>
      <div class="carousel-inner height-100" role="listbox">
               <div class="item height-100">
          <img class="first-slide" src="style/images/1.jpg" alt="First slide">
          <div class="container">
          <div class="carousel-caption">
               <p>A model school vision</p>
            </div>
          </div>
        </div>
        <div class="item height-100">
          <img class="first-slide" src="style/images/2.jpg" alt="First slide">
          <div class="container">
            <div class="carousel-caption">
               <p>Step towards healthy India</p>
            </div>
          </div>
        </div>
        <div class="item height-100 active left">
          <img class="second-slide" src="style/images/3.jpg" alt="Second slide">
          <div class="container">
            <div class="carousel-caption">
               <p>Empowering weavers through skill development</p>
            </div>
          </div>
        </div>
        <div class="item height-100 next left">
          <img class="third-slide" src="style/images/4.jpg" alt="Third slide">
          <div class="container">
            <div class="carousel-caption">
               <p>Developing Individuals through career guidance and counselling initiative</p>
            </div>
          </div>
        </div>
 <div class="item height-100">
 <div class="item height-100">
          <img class="fourth-slide" src="style/images/5.JPG" alt="Fourth slide">
          <div class="container">
            <div class="carousel-caption">
               <p>Our Mission is skill development. There can be no development with a satiated system </p>
            </div>
          </div>
        </div>

      </div>
      </div>';
?>
